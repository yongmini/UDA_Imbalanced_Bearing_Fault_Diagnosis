...
class dataset(object):
    ...
    def __init__(self, data_dir, dataset, faults, signal_size, normlizetype, condition=2):
        ...
        self.source_train_data, self.source_train_labels,self.source_test_data,self.source_test_labels = get_files(root=data_dir, dataset=dataset, faults=faults, signal_size=signal_size, condition=condition)
        self.transform = data_transforms(normlizetype)

    def data_prepare(self, imbalance_ratio=None, random_state=None):
        
        train_pd = pd.DataFrame({"data": self.source_train_data, "labels": self.source_train_labels})
        train_pd = data_utils.balance_data(train_pd, random_state=42)        
        
        val_pd = pd.DataFrame({"data": self.source_test_data, "labels": self.source_test_labels})
        ...
        source_counts = pd.concat([train_pd['labels'].value_counts(), val_pd['labels'].value_counts()], axis=1, keys=['train', 'val'])
        source_counts = source_counts.sort_index()
        source_counts.sort_index(inplace=True)
        source_counts.loc['Dataset Size'] = [len(train_dataset), len(val_dataset)]
        print("Source Data Distribution:")
        print(tabulate(source_counts, headers='keys', tablefmt='psql'))

        return train_dataset, val_dataset,train_pd['labels'].value_counts()
