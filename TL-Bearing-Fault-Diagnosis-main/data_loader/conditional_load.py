...
class dataset(object):
    ...
    def data_preprare(self, source_label=None, is_src=False, random_state=1):
        ...
        if is_src:
            ...
            train_counts = train_pd['labels'].value_counts()
            val_counts = val_pd['labels'].value_counts()

            train_counts_list = [[label, count] for label, count in train_counts.items()]
            train_counts_list.append(['Training Dataset Size', len(train_dataset)])
            val_counts_list = [[label, count] for label, count in val_counts.items()]
            val_counts_list.append(['Validation Dataset Size', len(val_dataset)])

            print("target unlabeld train Data Distribution:")
            print(tabulate(train_counts_list, headers=['Label', 'Count'], tablefmt='psql'))
            print("target validation Data Distribution:")
            print(tabulate(val_counts_list, headers=['Label', 'Count'], tablefmt='psql'))
